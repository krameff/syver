...
if [[ -n "${SYVER_BINARY:-}" ]]; then
  ...
  SYVER="$(mktemp -t syver-discovery-e2e.XXXXXX)"
...
cleanup() {
  if [[ "${SYVER:-}" == /tmp/syver-discovery-e2e.* ]] || [[ "${SYVER:-}" == *"/T/syver-discovery-e2e."* ]]; then
    rm -f "${SYVER}"
  fi
}
trap cleanup EXIT
...
run_discovery_e2e_steps "${EXAMPLES}" syver_runner
