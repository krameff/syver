...
Syver tests can be created by using either of following methods.

1. `syver autoadd <resource to test>`
2. `syver add <resource to test>`
3. manually create YAML/JSON test file by hand.

To customize the parameters generated by `syver add` and `syver autoadd` YAML file you need to manually edit it.

`syver add package nginx` will generate below YAML
...
Please note that using the `syver add` and `syver autoadd` command will create a valid file,
...
available at <https://raw.githubusercontent.com/krameff/syver/main/docs/schema.yaml>
...
* `schema url=https://raw.githubusercontent.com/krameff/syver/main/docs/schema.yaml`
...
This can be used in combination with the [gjson](#gjson) matcher to create powerful syver custom tests.
...
This could then be passed into syver: `syver --vars data.json validate`
...
!!! note
    `pid` is only populated when the OS lets syver map the socket back to its
...
!!! note
    `syver add`/discovery does not auto-populate `pid`, unlike the other
...
This is the most magical matcher for syver. It remains a default for historic and performance reasons.
Some attributes return an io.Reader that is read line by line (ex. file content, command, http body).
This allows syver to validate large files/content efficiently.
...
syver will attempt to transform the system state type before matching it.
...
```bash
syver --vars <(syver validate -g discovery.yaml --format discovery) validate -g goss.yml
```
...
```bash
syver validate -g discovery.yaml --format discovery > discovered.json
syver --vars discovered.json validate -g goss.yml --format documentation
```
...
Syver test files can leverage golang's [text/template](https://golang.org/pkg/text/template/)
...
!!! warning
    ...
    gossfiles containing text/template `{{}}` controls will no longer work with `syver add/autoadd`.
...
```bash
syver --vars <(ohai) validate
syver --vars <(facter -j) validate
```
...
# To validate:
$ OS=centos syver --vars vars.yaml validate
# To render:
$ OS=centos syver --vars vars.yaml render
# To render with debugging enabled:
$ OS=centos syver --vars vars.yaml render --debug
```
